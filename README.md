...
The `mbv` CLI is a thin wrapper — it doesn't render Markdown itself, it hands files to the Markdown Better View app, which must be installed. The app is a one-time **US$9.99** on the [Mac App Store](https://apps.apple.com/app/id6773928994) (macOS 15+, universal Apple Silicon + Intel), or a free 14-day trial via direct download:

```sh
brew install --cask markdownbetterview/tap/markdown-better-view
```

`brew install markdownbetterview/tap/mbv` installs just the `mbv` command (the app must be present separately).
